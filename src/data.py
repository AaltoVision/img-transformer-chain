...
def image_batch(image_paths, batch_size, load_size=286, crop_size=256, channels=3, shuffle=True,
                num_threads=4, min_after_dequeue=100, allow_smaller_final_batch=False, random_flip = True):
    ...
    '''
    HACK: tf.image.random_flip_left_right should be used before tf.image.resize_images,
    because tf.image.decode_image reutrns a tensor without shape which makes
    tf.image.resize_images collapse.

    This is a Tensorflow feature which certainly could be handled better here.
    '''
...
class ImageData:
    ...
    def __init__(self, session, image_paths, batch_size, load_size=286, crop_size=256, channels=3, shuffle=True,
        ...
        self.img_batch, self.img_num = image_batch(image_paths, batch_size, load_size, crop_size, channels, shuffle,
                                                   num_threads, min_after_dequeue, allow_smaller_final_batch, random_flip)
    ...
    def batch(self):
        self.lastBatch = self.sess.run(self.img_batch)
